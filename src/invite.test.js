...
describe("invite page", () => {
  it("ships a standalone static i.html entry with dedicated assets", () => {
    expect(existsSync(SHORT_ENTRY_HTML_PATH)).toBe(true);
    expect(existsSync(LEGACY_INVITE_HTML_PATH)).toBe(false);
    ...
    const inviteHtml = existsSync(SHORT_ENTRY_HTML_PATH)
      ? readFileSync(SHORT_ENTRY_HTML_PATH, "utf8")
      : "";
  ...
  it("registers i.html as a dedicated Vite build entry", () => {
    const viteConfig = readFileSync(resolve(process.cwd(), "vite.config.js"), "utf8");

    expect(viteConfig).toContain('resolve(__dirname, "i.html")');
    expect(viteConfig).not.toContain('resolve(__dirname, "invite/index.html")');
  });
